									...
									size_t len, loff_t *data)
{
	...
	size = min(sizeof(buf_tmp), len - 1);
	if (copy_from_user(buf_tmp, buf, size))
		return -EFAULT;
...
static ssize_t jadard_proc_display_read(struct file *file, char __user *buf,
											size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t *rdata = NULL;
 ...
 */
static ssize_t jadard_proc_display_write(struct file *file, const char __user *buf,
											size_t len, loff_t *pos)
{
