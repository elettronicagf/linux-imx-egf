...
static void jd9366t_PinReset(uint8_t reset_type)
{
...
JD_PRAM_CRC_RECHECK:
		...
		if (crc != MoveInfo[MoveInfoPsramIndex].to_mem_crc) {
			...
			if (pjadard_report_data->crc_start == 1) {
				if ((pjadard_ts_data->esd_check_running == false) || (pjadard_report_data->crc_retry >= 2)) {
...
static void module_jd9366t_PinReset(uint8_t reset_type)
{
	jd9366t_PinReset(reset_type);
}
